	...
	for(i = 0; i < size; i++) {
		...
		if (c != NULL) {
			...
			if (c->ops != NULL && c->ops->get_parent != NULL && c->ops->is_enabled != NULL && c->ops->is_enabled(c)) {
				...
				if (p_clk != NULL) {
					pr_info("[Acer Clock] Clock con_id=[%s] dev_id=[%s] is ON. (parent=%s)\r\n",
						msm_clocks_8960[i].con_id != NULL ? msm_clocks_8960[i].con_id : "null",
						msm_clocks_8960[i].dev_id != NULL ? msm_clocks_8960[i].dev_id : "null",
						p_clk->dbg_name);
				}
			}
		}
